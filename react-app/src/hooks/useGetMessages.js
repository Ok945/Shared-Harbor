    ...
    useEffect(() => {
        const getMessages = async () => {
            try {
                ...
                console.log("getMesage " ,selectedProduct , selectedConversation)
                const res = await axios.get(`http://localhost:8090/api/messages/?productId=${selectedProduct}&conversationId=${selectedConversation[0]._id}`, {
                    headers: {
        ...
        if (selectedConversation && selectedConversation[0] && selectedProduct) {
            setLoading(true);
